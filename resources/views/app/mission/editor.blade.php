...
<x-theme.galaxyhub.sub-content :title="$title" :description="$title" :breadcrumbs="Diglactic\Breadcrumbs\Breadcrumbs::render('app.mission', $title)">
    <x-panel.galaxyhub.basics>
        <div class="flex flex-col space-y-2" x-data="mission_create">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-4">
                <div class="">
                    ...
                    <div class="mt-1">
                        <x-input.select.basics id="type" name="type" x-model="data.create.body.type" required>
                ...
                <div class="pt-2 md:py-0">
                    ...
                    <div class="mt-1 grid grid-cols-2 gap-x-2">
                        <x-input.text.basics type="date" name="date" x-model="data.create.body.date" required/>
                        <x-input.text.basics type="time" name="date" x-model="data.create.body.time" step="1800" required/>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-x-4">
                <div class="">
                    ...
                    <div class="mt-1">
                        <x-input.select.basics id="type" name="map" x-model="data.create.body.map" required>
            ...
            <div class="">
                ...
                <textarea id="editor" name="body" class="hidden shadow-sm block w-full focus:ring-blue-500 focus:border-blue-500 sm:text-sm border border-gray-300 rounded-md" placeholder="미션 내용을 입력하여 주십시오. 예시) 냉전 / 아군:미군 적군:러시아군 / 클래스2 " x-model="data.create.body.body"></textarea>
            </div>
        ...
        <script type="text/javascript">
            window.document.addEventListener('alpine:init', () => {
                window.alpine.data('mission_create', () => ({
                    data: {
                        create: {
                            url: @if($edit) '{{route('mission.update')}}' @else '{{route('mission.create')}}' @endif,
            ...
            window.addEventListener('load', function(){
                ClassicEditor.create(document.querySelector('#editor'), {
                    ...
                    simpleUpload: {
                        uploadUrl: '{{route('file.upload.ckeditor', 'mission')}}',
                        headers: {
